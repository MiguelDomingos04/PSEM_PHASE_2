...
void SpeedProducer::setupMCPWM()
{
    ...
    chan_cfg.gpio_num       = CAP_GPIO;               // GPIO onde o sensor de velocidade está conectado. Este GPIO deve ser capaz de funcionar como entrada para o timer de captura do MCPWM. O GPIO4 é uma escolha comum para este tipo de aplicação.
    chan_cfg.prescale       = 1;                      // Prescaler do timer de captura.
    chan_cfg.flags.pos_edge = true;                  // Captura na borda positiva (subida) do sinal. O sensor de velocidade geralmente gera um pulso para cada evento de medição (por exemplo, cada vez que uma marca no eixo passa pelo sensor), e queremos capturar o tempo desses eventos para calcular a velocidade.
    chan_cfg.flags.neg_edge = true;                  // Captura na borda negativa (descida) do sinal. Capturar tanto a borda positiva quanto a negativa permite medir o período total do sinal e o tempo em que o sinal está alto, o que pode ser útil para calcular a velocidade com base em pulsos de largura variável.
    chan_cfg.flags.pull_up  = true;                  // Ativa o pull-up interno no
